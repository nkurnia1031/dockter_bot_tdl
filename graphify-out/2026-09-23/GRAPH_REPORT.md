...
## Corpus Check
- 158 files · ~121,240 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 9 file(s) not represented in the graph (top: (none) 6, .base 1, .conf 1)

## Summary
- 2467 nodes · 6026 edges · 108 communities (82 shown, 23 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 329 edges (avg confidence: 0.92)
...
Cohesion: 0.11
Nodes (22): LeaveResult, CommandCallback, has_downloadable_media(), is_image_message(), Any, BatchDownloadResult, BatchDownloadService, build_telegram_message_url() (+14 more)
...
Cohesion: 0.07
Nodes (26): json_value(), Any, Exception, Path, Return a per-stage TDL client, with a test/runtime fallback. Production profile…, Reconstruct Quick Mode metadata when the backend manifest is gone., Keep a recovery copy of the raw TDL export in Quick Mode staging., Create a temporary workspace-only input for the download service. (+18 more)
...
Cohesion: 0.07
Nodes (6): QuickPipelineTests, download_export_to(), run(), ProgressReporter, Throttled current-state telemetry plus persistent milestone events., UtilityResult

### Community 35 - "ExportArtifactCatalog"
Cohesion: 0.12
Nodes (9): ExportArtifactCatalogTests, ExportArtifactCatalog, Any, Connection, Gateway-owned catalog for export JSON artifacts. The worker owns the physical…, Upsert an inventory batch in one SQLite transaction. Inventory can contain…, Mark catalog rows absent when a worker inventory completes., Remove a missing file from the runnable queue. The physical file is already… (+1 more)
...
Cohesion: 0.05
Nodes (37): BackupServiceTests, make_config(), Path, FakePublisher, ProgressReporterTests, BackupArchive, BackupService, Path (+29 more)
...
### Community 42 - "TDLClient"
Cohesion: 0.06
Nodes (25): OutputCallback, ProgressCallback, FakeRunner, CompletedProcess, TDLClientTests, decode_process_output(), _message_contains_caption(), visit() (+17 more)
...
Cohesion: 0.13
Nodes (11): FakeSubprocessRunner, RcloneRunnerTests, CommandCallback, Path, RuntimeError, Raised when an rclone transfer cannot be completed., Verify exact remote files without downloading or mutating them., Small, cancellable rclone adapter for files already in the workspace. (+3 more)
...
### Community 54 - "inspect_export_json"
Cohesion: 0.32
Nodes (5): discard_export_without_media(), inspect_export_json(), Path, Return safe media statistics without assuming a single TDL JSON shape., Remove an export JSON when its inspected media count is exactly zero. The…
...
Cohesion: 0.14
Nodes (18): AppConfig, Fail fast when a production role is missing its trust boundary., LeaveService, normalize_profile_name(), build_profile_config(), build_profile_runtime(), chown_paths(), chown_tree() (+10 more)
...
Cohesion: 0.25
Nodes (5): sha256_file(), _has_transfer_telemetry(), part_progress(), export_progress(), upload_progress()
...
Cohesion: 0.09
Nodes (44): StorageWorkerPathTests, _quick_message_matches(), Match a storage message without persisting or returning its contents., Verify completed Quick Mode uploads before deleting retained staging., Return nested directories, including empty ones, as portable paths., storage_logical_folder(), storage_relative_folders(), _utility_progress_message() (+36 more)
...
### Community 110 - "._write_quick_log_line"
Cohesion: 0.21
...
- **Why does `WorkerJobExecutor` connect `WorkerJobExecutor` to `ProgressReporter`, `BatchDownloadService`, `tme3bot/app.py`, `BackupService`, `composition.py`, `BackendApiTests`, `._backup`, `WorkerEventPublisher`, `test_backend_api.py`, `executor.py`, `._write_quick_log_line`, `ExportJobResult`, `QuickThumbnailBuilder`, `RcloneRunner`, `inspect_export_json`, `ResourceAwareQueue`, `._recover_legacy_quick_stages`, `QuickThumbnailTests`?**
...
- **Why does `create_backend_app()` connect `create_backend_app` to `JobEvent`, `tme3bot/app.py`, `test_backend_api.py`, `Job`, `composition.py`, `DomainError`, `StorageCatalog`, `browser_refresh`, `_error`, `sign_storage_item`, `.setUp`, `ControlPlane`, `backend.py`?**
