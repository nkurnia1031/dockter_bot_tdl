...
## Corpus Check
- 146 files · ~91,625 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1922 nodes · 4970 edges · 84 communities (69 shown, 15 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 455 edges (avg confidence: 0.53)
...
Cohesion: 0.17
Nodes (5): Thread, Any, Recover subscriptions after the Telegram container restarts., Telegram presentation adapter; all business actions use BackendApiClient., TelegramFrontendApp
...
Cohesion: 0.11
Nodes (13): UtilitySettingsTests, UtilitySummaryTests, Path, ValueError, Public metadata for clients; never contains a setting value or secret., Remove the two intermediate JSON files produced by ``pindah``. ``pindah4.py``…, utility_setting_specs(), UtilityPathError (+5 more)
...
Cohesion: 0.15
Nodes (23): PendingInput, Telegram presentation adapter and UI-only helpers., backup_menu_markup(), clear_confirm_markup(), download_status_markup(), export_input_cancel_markup(), _export_source_picker_markup(), main_menu_markup() (+15 more)

### Community 21 - "StateStore"
Cohesion: 0.19
Nodes (9): FakeDownloadTDLClient, FakeExportTDLClient, Path, ServiceTests, StateStoreTests, ExportService, Path, StateStore (+1 more)

### Community 22 - "WorkerJobExecutor"
Cohesion: 0.06
Nodes (30): FakePublisher, ProgressReporterTests, StorageWorkerPathTests, sha256_file(), discard_export_without_media(), Remove an export JSON when its inspected media count is exactly zero. The…, JsonHttpError, _progress_percent() (+22 more)

### Community 24 - "Update"
Cohesion: 0.18
Nodes (5): CallbackContext, Exception, Accept a signed file code without requiring an application actor., check_profile_markup(), Update
...
Cohesion: 0.12
Nodes (11): ExportArtifactCatalogTests, ExportArtifactCatalog, inspect_export_json(), Any, Connection, Path, Return safe media statistics without assuming a single TDL JSON shape., Upsert an inventory batch in one SQLite transaction. Inventory can contain… (+3 more)
...
### Community 37 - "TDLCommandError"
Cohesion: 0.11
Nodes (16): OutputCallback, Popen, ProgressCallback, LeaveResult, LeaveService, _normalize_upload_caption(), CompletedProcess, Path (+8 more)
...
Cohesion: 0.07
Nodes (20): FakeExecutor, WorkerApiTests, create_worker_app(), _error(), FastAPI, JSONResponse, Request, WorkerContext (+12 more)

### Community 46 - "control_plane.py"
Cohesion: 0.31
Nodes (5): Application services and use cases., build_execution_plan(), JobExecutionPlan, Any, Internal scheduling metadata shared by backend and worker.
...
Cohesion: 0.09
Nodes (5): FakeDispatcher, FakeProfiles, FakeTelegramBot, FakeWorkers, Gateway-owned catalog for export JSON artifacts. The worker owns the physical…
...
Cohesion: 0.09
Nodes (19): ExportWorkspaceTests, export_report(), ExportWorkspaceState, ExportWorkspaceStore, format_export_job(), format_export_status(), format_rate(), is_numeric_chat_ref() (+11 more)

### Community 54 - "DomainError"
Cohesion: 0.14
Nodes (13): Enum, str, FailingDispatcher, FakeDispatcher, FakeProfiles, Framework-independent domain model for the tme3bot control plane., AuthChallengeStatus, DomainError (+5 more)
...
### Community 57 - "tdl.py"
Cohesion: 0.14
Nodes (23): Queue, _message_contains_caption(), _byte_multiplier(), clean_tdl_output_line(), _duration_seconds(), is_nonsemantic_tdl_output_line(), is_standalone_tdl_progress_bar(), is_tdl_telemetry_line() (+15 more)
...
## Knowledge Gaps
- **162 isolated node(s):** `tme3bot-leave-helper`, `compress.sh script`, `pindah.sh script`, `name`, `version` (+157 more)
...
- **Why does `TelegramFrontendApp` connect `TelegramFrontendApp` to `test_app_menu.py`, `PanelManager`, `request_json`, `format_job_status`, `telegram/app.py`, `ExportWorkspaceState`, `WorkerJobExecutor`, `FakeStatusPanel`, `Update`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `PanelManager` connect `PanelManager` to `telegram/app.py`, `TelegramFrontendApp`?**
...
- **Are the 58 inferred relationships involving `BackendContext` (e.g. with `BackendApiTests` and `FakeDispatcher`) actually correct?**
  _`BackendContext` has 58 INFERRED edges - model-reasoned connections that need verification._
