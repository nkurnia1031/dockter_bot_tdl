...
Cohesion: 0.07
Nodes (22): json_value(), Any, Exception, Path, Remove stale progress redraws while retaining diagnostic lines., Return a per-stage TDL client, with a test/runtime fallback. Production profile…, Reconstruct Quick Mode metadata when the backend manifest is gone., Keep a recovery copy of the raw TDL export in Quick Mode staging. (+14 more)
...
Cohesion: 0.06
Nodes (27): BackupServiceTests, make_config(), Path, UtilitySettingsTests, UtilitySummaryTests, BackupArchive, BackupService, Path (+19 more)
...
Cohesion: 0.11
Nodes (15): OutputCallback, ProgressCallback, LeaveResult, LeaveService, CommandCallback, notify_command_completed(), notify_command_started(), CommandCallback (+7 more)
...
Cohesion: 0.08
Nodes (21): sha256_file(), _has_transfer_telemetry(), _quick_message_matches(), Run the post-export Quick Mode stages while retaining staging on error., Upload workspace files through the worker-local rclone config., Verify completed Quick Mode uploads before deleting retained staging., Match a storage message without persisting or returning its contents., part_progress() (+13 more)
...
### Community 104 - "storage_logical_folder"
Cohesion: 0.43
...
Cohesion: 0.11
Nodes (34): jakarta_timestamp(), Return human-facing worker log timestamps in the project timezone., _utility_progress_message(), utility_progress(), Framework-independent worker execution adapter., _archive_files(), _chown_tree(), cleanup_quick_stage() (+26 more)
...
- **Why does `WorkerJobExecutor` connect `WorkerJobExecutor` to `test_quick_export.py`, `._quick_export_pipeline`, `tme3bot/app.py`, `utility.py`, `composition.py`, `WorkerEventPublisher`, `ProgressReporter`, `SubprocessRunner`, `._worker_heartbeat_loop`, `executor.py`, `QuickThumbnailBuilder`, `RcloneRunner`, `ResourceAwareQueue`, `Path`?**
