...
## Corpus Check
- 155 files · ~108,030 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 9 file(s) not represented in the graph (top: (none) 6, .base 1, .conf 1)

## Summary
- 2317 nodes · 5623 edges · 117 communities (88 shown, 25 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 311 edges (avg confidence: 0.92)
...
3. `DomainError` - 84 edges
4. `TelegramFrontendApp` - 76 edges
5. `WorkerJobExecutor` - 73 edges
...
Cohesion: 0.18
Nodes (11): LeaveResult, LeaveService, normalize_profile_name(), chown_paths(), chown_tree(), ensure_profile_runtime_dirs(), get_profile_download_mode(), profile_settings_path() (+3 more)
...
Cohesion: 0.05
Nodes (28): create_backend_app(), archive_artifact(), archive_job(), cancel_job(), clear_failed(), create_telegram_notification(), delete_artifact_file(), delete_artifacts_batch() (+20 more)

### Community 18 - "UtilityRunner"
Cohesion: 0.07
Nodes (20): UtilitySettingsTests, UtilitySummaryTests, Path, RuntimeError, Raised when an rclone transfer cannot be completed., RcloneError, Path, Popen (+12 more)
...
Cohesion: 0.08
Nodes (27): json_value(), Any, Exception, Path, Run the post-export Quick Mode stages while retaining staging on error., Executes domain jobs and publishes JSON events; no UI dependency., Upload workspace files through the worker-local rclone config., Return a safe, shallow directory listing for the active workspace. (+19 more)
...
Cohesion: 0.08
Nodes (26): ExportMilestoneTests, QuickPipelineTests, download_export_to(), run(), ProgressReporter, Throttled current-state telemetry plus persistent milestone events., DownloadedJsonResult, ExportJobResult (+18 more)
...
### Community 42 - "TDLCommandError"
Cohesion: 0.17
Nodes (10): FakeSubprocessRunner, RcloneRunnerTests, Small, cancellable rclone adapter for files already in the workspace., RcloneRunner, RuntimeError, Raised when a TDL subprocess stops making semantic progress., Raised when TDL returns unusable or malformed export data., TDLCommandError (+2 more)
...
### Community 52 - "ExportService"
Cohesion: 0.33
Nodes (3): build_telegram_message_url(), ExportService, ParsedTme3Url
...
Cohesion: 0.12
Nodes (6): OutputCallback, ProgressCallback, Queue, Any, Popen, SubprocessRunner

### Community 55 - "TDLClient"
Cohesion: 0.19
Nodes (5): FakeRunner, CompletedProcess, TDLClientTests, decode_process_output(), TDLClient
...
### Community 58 - "test_backend_api.py"
Cohesion: 0.12
Nodes (10): FakeTelegramBot, BotAuthService, _hash_secret(), _now(), Any, Connection, datetime, Path (+2 more)
...
Cohesion: 0.11
Nodes (9): FailingDispatcher, FakeDispatcher, FakeProfiles, serializable(), Application services and use cases., build_execution_plan(), JobExecutionPlan, Any (+1 more)

### Community 70 - "composition.py"
Cohesion: 0.27
Nodes (4): build_backend_context(), ControlPlaneBackupRouter, _first_actor(), _NullCoordinator

### Community 71 - "parse_tme3_url"
Cohesion: 0.30
...
Cohesion: 0.26
Nodes (7): Enum, str, Framework-independent domain model for the tme3bot control plane., AuthChallengeStatus, JobStatus, _quick_mode_sql(), Filter the compact JSON payload without requiring SQLite JSON1.
...
### Community 84 - "inspect_export_json"
Cohesion: 0.18
Nodes (10): discard_export_without_media(), inspect_export_json(), Path, Gateway-owned catalog for export JSON artifacts. The worker owns the physical…, Return safe media statistics without assuming a single TDL JSON shape., Remove an export JSON when its inspected media count is exactly zero. The…, has_downloadable_media(), is_image_message() (+2 more)
...
### Community 87 - ".upload"
Cohesion: 0.23
Nodes (9): _message_contains_caption(), visit(), _normalize_upload_caption(), CompletedProcess, Path, Match captions across the different JSON shapes emitted by TDL., Upload one file and optionally force it to Telegram photo media., Resolve delayed TDL upload results by polling channel history. Some TDL… (+1 more)
...
Cohesion: 0.16
Nodes (12): Path, QuickThumbnailTests, fake_run(), fake_run(), fake_run(), Path, Popen, QuickThumbnailBuilder (+4 more)
...
### Community 102 - "AuthServiceTests"
Cohesion: 0.46
Nodes (3): AuthServiceTests, actor(), Path

### Community 105 - "progress_reporter.py"
Cohesion: 0.31
...
### Community 108 - "storage_logical_folder"
Cohesion: 0.43
...
- **Why does `create_backend_app()` connect `create_backend_app` to `JobEvent`, `composition.py`, `sign_storage_item`, `DomainError`, `AppConfig`, `browser_refresh`, `JobStatus`, `StorageCatalog`, `_error`, `.setUp`, `test_backend_api.py`, `ControlPlane`, `backend.py`, `storage_item_dict`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `WorkerJobExecutor` connect `WorkerJobExecutor` to `executor.py`, `._backup`, `JobLogSnapshot`, `composition.py`, `WorkerEventPublisher`, `TDLCommandError`, `AppConfig`, `._recover_legacy_quick_stages`, `.test_export_publishes_json_ready_milestone_before_post_export_cleanup`, `.test_quick_cancel_is_best_effort_when_some_runtimes_are_already_gone`, `.test_export_retry_passes_message_id_range_to_export_service`, `BackupService`, `UtilityRunner`, `inspect_export_json`, `SubprocessRunner`, `ResourceAwareQueue`, `QuickThumbnailBuilder`?**
