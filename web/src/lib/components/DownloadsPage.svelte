<script lang="ts">
  ...
  const activeWorker = $derived(session.current.actor?.worker_route || '');
  const isAvailable = (item: Artifact) => item.available === undefined || Boolean(item.available);
  const visible = $derived(artifacts.filter((item) =>
    activeTab === 'pending'
      ? isAvailable(item) && ['pending','failed'].includes(item.status)
      : activeTab === 'processing'
        ? isAvailable(item) && item.status === 'processing'
        : activeTab === 'history'
          ? (!isAvailable(item) || ['downloaded','failed','deleted'].includes(item.status)) && !item.archived_at
  ...
  async function loadArtifacts(archived: boolean, worker: string): Promise<Artifact[]> {
    const result: Artifact[] = [];
    let offset = 0;
    while (true) {
      const page = await api<{items: Artifact[]; total?: number}>(
        `/downloads/artifacts?limit=${ARTIFACT_PAGE_SIZE}&offset=${offset}&archived=${archived}&worker=${encodeURIComponent(worker)}`
      );
  ...
  async function load(worker = activeWorker, request = contextGeneration) {
    if (!worker) return;
    try {
      const [current, archived] = await Promise.all([loadArtifacts(false, worker), loadArtifacts(true, worker)]);
      if (request !== contextGeneration || worker !== activeWorker) return;
      ...
      message = cause instanceof Error ? cause.message : 'Artifact gagal dimuat.';
    }
  }
  function toggle(item: Artifact) {
    if (selected.includes(item.id)) selected = selected.filter((id) => id !== item.id);
    else if (!isAvailable(item)) message = 'File artifact sudah tidak tersedia pada worker.';
  ...
  async function startAll() {
    const ids = artifacts
      .filter((item) => item.status === 'pending' && isAvailable(item) && (item.worker || 'local') === activeWorker)
      .map((item) => item.id);
    if (!ids.length) { message = `Tidak ada artifact tersedia pada worker ${activeWorker}.`; return; }
    pending = true;
    try { await post('/downloads', { artifact_ids: ids, priority: 'normal' }); message = `${ids.length} artifact masuk antrean ${activeWorker}.`; await load(); }
  ...
  async function reconcile() {
    ...
    artifacts = [];
    selected = [];
    message = `Menyinkronkan artifact dari worker ${worker}...`;
    try {
      const inventory = await post<{id:string}>('/downloads/artifacts/reconcile');
      while (request === contextGeneration && worker === activeWorker) {
        const job = await api<{status:string;error?:{message?:string}}>(`/jobs/${inventory.id}`);
        if (['succeeded','failed','cancelled'].includes(job.status)) {
          if (job.status !== 'succeeded') throw new Error(job.error?.message || 'Inventory worker gagal.');
          break;
        }
        await wait(750);
      }
      if (request !== contextGeneration || worker !== activeWorker) return;
      await load(worker, request);
      message = '';
    } catch (cause) {
      if (request !== contextGeneration) return;
      artifacts = [];
      message = cause instanceof Error ? cause.message : `Worker ${worker} tidak dapat disinkronkan.`;
    } finally {
      if (request === contextGeneration) {
        syncing = false;
        pending = false;
      }
    }
  ...
  onMount(() => {
    const contextRefresh = () => reconcile();
...
<section class="card mt-7 overflow-hidden">
  <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] p-4 sm:p-5">
    ...
    <div class="flex flex-wrap items-center gap-2"><span class="badge running">{activeWorker || '-'}</span><button class="button secondary" onclick={reconcile} disabled={pending}><RefreshCw class={syncing ? 'animate-spin' : ''} size={15}/>Reconcile</button>{#if activeTab === 'pending'}<button class="button secondary" onclick={clearFailed} disabled={pending || syncing}><Trash2 size={15}/>Clear failed</button><button class="button secondary" onclick={startAll} disabled={pending || syncing}><Play size={15}/>Mulai semua</button>{/if}</div>
  </div>
  {#if activeTab === 'pending' && selected.length}<div class="flex flex-wrap items-center gap-2 border-b border-[var(--line)] bg-violet-50 p-3 dark:bg-violet-950"><b class="mr-auto text-sm">{selected.length} dipilih · {selectedWorker}</b><button class="button" onclick={() => start(selected)} disabled={pending || syncing}><Download size={15}/>Mulai</button><button class="button secondary" onclick={() => start(selected, 'next')} disabled={pending || syncing}><FastForward size={15}/>Berikutnya</button>{#if selectedItems.some((item) => item.status === 'failed')}<button class="button secondary" onclick={() => start(selected, 'normal', true)} disabled={pending || syncing}><RotateCcw size={15}/>Retry</button>{/if}</div>{/if}
  ...
  <div class="overflow-x-auto"><table class="w-full min-w-[900px] text-left text-sm"><thead class="border-b border-[var(--line)] text-xs uppercase text-[var(--muted)]"><tr><th class="p-4">Pilih</th><th>Artifact</th><th>Statistik</th><th>Ukuran</th><th>Worker/status</th><th>Waktu</th><th class="pr-4 text-right">Aksi</th></tr></thead><tbody class="divide-y divide-[var(--line)]">
    {#each visible as item}<tr class="hover:bg-[var(--brand-soft)]/35"><td class="p-4"><input type="checkbox" disabled={pending || syncing || !isAvailable(item) || !['pending','failed'].includes(item.status)} checked={selected.includes(item.id)} onchange={() => toggle(item)}/></td><td><b>{item.filename}</b><p class="muted mt-1">{item.label || item.chat_ref || '-'}</p>{#if item.error}<p class="mt-1 max-w-72 truncate text-xs text-rose-600">{item.error}</p>{/if}</td><td>{item.message_count ?? '?'} pesan<br/><span class="muted">{item.photo_count ?? '?'} foto · {item.video_count ?? '?'} video · {item.media_count ?? '?'} media</span></td><td>{formatBytes(item.actual_downloaded_bytes ?? item.expected_media_bytes ?? item.json_bytes)}</td><td><b>{item.worker || 'local'}</b><br/><span class={`badge ${item.status}`}>{item.status}</span>{#if !isAvailable(item)}<span class="badge failed ml-1">File tidak tersedia</span>{/if}</td><td>{formatDate(item.completed_at || item.created_at)}</td><td class="pr-4"><div class="flex justify-end gap-1">{#if !isAvailable(item)}<span class="muted text-xs">Tidak ada aksi</span>{:else if ['pending','failed'].includes(item.status)}<button class="button secondary" onclick={() => start([item.id])} disabled={pending || syncing}><Play size={14}/>Mulai</button><button class="button ghost text-rose-600" onclick={() => confirm = {action:'delete',item}} disabled={pending || syncing}><Trash2 size={15}/></button>{:else if activeTab === 'archived'}<button class="button secondary" onclick={() => restore(item)}><RotateCcw size={14}/>Restore</button><button class="button danger" onclick={() => confirm = {action:'purge',item}}><Trash2 size={14}/>Purge</button>{:else}<button class="button secondary" onclick={() => archive(item)}><Archive size={14}/>Archive</button>{/if}</div></td></tr>
...
<JobTable kind="download" title="Progress & riwayat download" worker={activeWorker}/>
