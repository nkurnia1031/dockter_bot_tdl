<script lang="ts">
  ...
  async function load() {
    try {
      jobs = (await api<{items:Job[]}>(`/jobs?limit=30${kind ? `&kind=${encodeURIComponent(kind)}` : ''}`)).items || [];
      error = '';
    } catch (cause) { error = cause instanceof Error ? cause.message : 'Tidak dapat memuat job.'; }
    finally { if (mounted) schedule(); }
  }
  async function loadEvents() {
    if (!selected) return;
    loadingEvents = true;
    try { events = (await api<{items:JobEvent[]}>(`/jobs/${selected.id}/events?after_sequence=0`)).items || []; }
    catch (cause) { error = cause instanceof Error ? cause.message : 'Tidak dapat memuat event.'; }
    finally { loadingEvents = false; }
  }
  async function openReport(job: Job) { selected = job; events = []; detailTab = 'summary'; reportOpen = true; await loadEvents(); }
