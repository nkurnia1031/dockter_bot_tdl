...
export function ActivityPage() {
  ...
  const jobs = useQuery<{items: Job[]; total: number}>({
    queryKey: ["activity", status],
    queryFn: () => api(`/jobs?limit=100&archived=false${status ? `&status=${status}` : ""}`),
    refetchInterval: 3000,
  });
  return <>
    <PageHeader eyebrow="Audit trail" title="Seluruh aktivitas" description="Riwayat persisten lintas Telegram dan web pada profile aktif."/>
    <Card>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-bold">{jobs.data?.total || 0} job</h2>
        <div className="flex items-center gap-2">
          <TerminateAllJobsButton/>
          <select className={`${inputClass} w-44`} value={status} onChange={(event) => {
            const url = new URL(location.href);
            event.target.value ? url.searchParams.set("status", event.target.value) : url.searchParams.delete("status");
            location.href = url.toString();
          }}>
            <option value="">Semua status</option><option>queued</option><option>dispatched</option><option>running</option><option>succeeded</option><option>failed</option><option>cancelled</option>
          </select>
        </div>
      </div>
